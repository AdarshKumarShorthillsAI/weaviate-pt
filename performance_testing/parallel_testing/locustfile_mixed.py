"""
PARALLEL MIXED SEARCH - Locust performance testing for Weaviate
Sends 9 PARALLEL requests with rotating search types (BM25, Hybrid 0.1, Hybrid 0.9, Vector)
...
def on_locust_init(environment, **kwargs):
    """Load parallel mixed query file when Locust starts"""
    global QUERIES
    
    filename = "queries/queries_mixed_200.json"
    
    print("=" * 80)
    print("🚀 PARALLEL MIXED SEARCH TEST")
    print("=" * 80)
    print(f"Loading: {filename}")
    print(f"Execution: 9 parallel requests (one per collection)")
    print(f"Collections: {', '.join(COLLECTIONS)}")
    print(f"Search Types: Rotates through BM25, Hybrid 0.1, Hybrid 0.9, Vector")
    print("=" * 80)
    ...
    try:
        ...
        print("   Run: python generate_parallel_queries.py --search-types mixed")
        print("=" * 80)


class ParallelMixedUser(HttpUser):
    """Simulates a user performing parallel mixed searches across 9 collections"""
    ...
    def parallel_search_all_collections(self):
        """Execute parallel mixed search across all 9 collections"""
        if not QUERIES:
            return
        ...
        if successful == 9:
            ...
            events.request.fire(
                ...
                name="Parallel_Mixed_All_9_Collections",
            ...
            events.request.fire(
                ...
                name=f"Parallel_Mixed_Partial_{successful}_of_9",
                response_time=total_time,
                response_length=0,
            ...
            events.request.fire(
                request_type="FAILED",
                name="Parallel_Mixed_All_Failed",
                response_time=total_time,
                response_length=0,
