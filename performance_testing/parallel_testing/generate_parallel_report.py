...
def scan_parallel_results(results_folder):
    ...
    for test_prefix, search_type in test_mapping.items():
        ...
        pattern = os.path.join(results_folder, f"{test_prefix}_limit*_stats.csv")
...
def main():
    ...
    html = generate_html_report(results, results_folder)
    
    # Save report
    output_file = 'parallel_collection_report.html'
