"""
PARALLEL HYBRID SEARCH (Alpha=0.9) - Locust performance testing for Weaviate
Sends 9 PARALLEL hybrid requests with alpha=0.9 (10% vector, 90% BM25)
...
def on_locust_init(environment, **kwargs):
    """Load parallel hybrid (alpha=0.9) query file when Locust starts"""
    global QUERIES
    
    filename = "queries/queries_hybrid_09_200.json"
    
    print("=" * 80)
    print("🚀 PARALLEL HYBRID SEARCH TEST (Alpha=0.9)")
    print("=" * 80)
    print(f"Loading: {filename}")
    print(f"Execution: 9 parallel requests (one per collection)")
    print(f"Collections: {', '.join(COLLECTIONS)}")
    print(f"Weight: 10% vector + 90% BM25")
    print("=" * 80)
    ...
    try:
        ...
        print("   Run: python generate_parallel_queries.py --search-types hybrid_09")
        print("=" * 80)


class ParallelHybrid09User(HttpUser):
    """Simulates a user performing parallel hybrid (0.9) searches across 9 collections"""
    ...
    def execute_single_query(self, query_data):
        ...
        try:
            response = self.client.post(
                ...
                timeout=30,
                name=f"Parallel_Hybrid09_{query_data['collection']}"
            )
    ...
    def parallel_search_all_collections(self):
        """Execute parallel hybrid (0.9) search across all 9 collections"""
        if not QUERIES:
            return
        ...
        if successful == 9:
            ...
            events.request.fire(
                ...
                name="Parallel_Hybrid09_All_9_Collections",
            ...
            events.request.fire(
                ...
                name=f"Parallel_Hybrid09_Partial_{successful}_of_9",
                response_time=total_time,
                response_length=0,
            ...
            events.request.fire(
                request_type="FAILED",
                name="Parallel_Hybrid09_All_Failed",
                response_time=total_time,
                response_length=0,
