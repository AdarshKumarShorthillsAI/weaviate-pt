"""
PARALLEL VECTOR SEARCH - Locust performance testing for Weaviate
Sends 9 PARALLEL requests (one per collection) and waits for all to complete.

This tests: "Is parallel execution faster than single multi-collection query?"
...
def on_locust_init(environment, **kwargs):
    ...
    print("=" * 80)
    print("🚀 PARALLEL VECTOR SEARCH TEST")
    print("=" * 80)
...
class ParallelVectorUser(HttpUser):
    """Simulates a user performing parallel vector searches across 9 collections"""
    ...
    def parallel_search_all_collections(self):
        """Execute parallel vector search across all 9 collections"""
        if not QUERIES:
            return
        ...
        if successful == 9:
            ...
            events.request.fire(
                ...
                name="Parallel_Vector_All_9_Collections",
            ...
            events.request.fire(
                ...
                name=f"Parallel_Vector_Partial_{successful}_of_9",
                response_time=total_time,
                response_length=0,
            ...
            events.request.fire(
                request_type="FAILED",
                name="Parallel_Vector_All_Failed",
                response_time=total_time,
                response_length=0,
